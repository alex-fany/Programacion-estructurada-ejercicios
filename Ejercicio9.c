...
PROGRAMA: ejercicio9.c
CENTRO UNIVERSITARIO DE LOS ALTOS / UNIVERSIDAD DE GUADALAJARA
INGENIERÍA EN COMPUTACIÓN / 2DO SEMESTRE
PROFESOR: Carlos Javier Cruz Franco
DESCRIPCIÓN: Ordenar valores de vector con algoritmos
**************************************************/
	...
	scanf("%d", &b);
		if(b==1){ //Opción 1: Ver valores guardados en vector
			...
			switch(c){ //Según la opción seleccionada
				case 1: //Con algoritmo de ordenamiento Bubble, llamar void
					bubble(vect, a);
				break;
				case 2: //Con algoritmo de ordenamiento QuickSort, llamar void
					quicksort(vect, 0, a-1);
				break;
			}printf("\nVector ordenado:\n"); //Muestra vector ordenado al final del switch
			for(i=1; i<=a; i++){
				printf("| %i- ", i);
				printf("%i |", vect[i]);
			}
